...
#------------------------------------------------------------------------------
# elasticsearch
RUN cd /tmp && curl -LO https://download.elastic.co/elasticsearch/release/org/elasticsearch/distribution/deb/elasticsearch/2.4.1/elasticsearch-2.4.1.deb && sudo dpkg -i elasticsearch-2.4.1.deb &&  rm elasticsearch-2.4.1.deb && sudo ln -s /etc/elasticsearch/ /usr/share/elasticsearch/config
...
RUN cd /tmp && curl -O http://download.redis.io/redis-stable.tar.gz && tar xzvf redis-stable.tar.gz && cd redis-stablemake && make install
